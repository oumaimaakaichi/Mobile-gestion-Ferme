...
import animals from "../../assets/v-removebg-preview.png";
...
export default function AddVeterinaire({ navigation }) {
  ...
  const addOuvrier = async () => {
    ...
    try {
      ...
      const response = await fetch("http://192.168.195.216:3000/add-CompteV", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        Toast.show({
          position: "top",
          type: "success",
          text1: "Ajouter un veterinaire",
          text2: "Veterinaire ajouté avec succès",
          autoHide: true,
          visibilityTime: 3000,
          onHide: () => {
